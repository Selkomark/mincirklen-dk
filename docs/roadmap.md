...
- Bots / sybil accounts / AI-generated participants: **superseded by the shipped design — see the note below the original brainstorm.** *(Original framing, kept for history: the anonymity requirement makes this the hardest problem on the list, because most bot-prevention tech — government ID, biometrics, phone-linked accounts — directly conflicts with the anonymity promise. The practical middle ground in 2026 is proof-of-personhood without identity disclosure — approaches like zero-knowledge personhood credentials (e.g., Semaphore-based schemes), passkey/WebAuthn device attestation, or a one-time phone-number check that's hashed and immediately discarded, never stored, never linked to the pseudonym. None of these are perfect; combine 2–3 weak signals rather than betting on one strong one.)* What actually shipped is simpler than any of this: mandatory Google OAuth sign-in plus a completed profile (name, country, mobile number) is the platform's real identity boundary — `verifiedProcedure` in `services/trpc-api/src/controllers/trpc.ts` blocks every real feature (create/join a circle, send a message) until both are done. None of that identity is ever shown publicly or to other users — see CHARTER.md §4 — so "anonymity" here means *pseudonymous and non-public*, not *unverified*. An earlier build briefly supported a bare pre-Google "anonymous session" (`auth.createAnonymousSession`); it was removed (2026-09-10) once it became clear every real code path already required full verification, so it bought no product value while being the platform's cheapest, least-gated abuse door — see `SECURITY_FINDINGS.md` H1.
